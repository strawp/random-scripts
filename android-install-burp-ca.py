...
print('')
print('Getting hash')
certhash = subprocess.check_output('openssl x509 -inform PEM -subject_hash_old -in burp.pem | head -1', shell=True).decode('utf8').strip()
print('Hash: ' + str(certhash))
certhash = str(certhash) + '.0'

# Rename the burp.Pem to <hash>.0
# 
# mv burp.pem <hash>.0
print('')
print('Renaming to ' + str(certhash))
shutil.move( 'burp.pem', str(certhash))
...
subprocess.call('adb push ' + str(certhash) + ' /sdcard/Download', shell=True )
...
subprocess.call("adb shell su root 'mv /sdcard/Download/"+str(certhash)+" /system/etc/security/cacerts'", shell=True )

# chmod 644 /system/etc/security/cacerts/<hash>.0
print('')
print('Changing permissions to 644')
subprocess.call("adb shell su root 'chmod 644 /system/etc/security/cacerts/"+str(certhash)+"'", shell=True )
